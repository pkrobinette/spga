...
def main():
    ...
    for i in range(args.num_trials):
        ...
        env = Knapsack(env_config)
        agent = Generation(env=env, n=100, g=args.max_steps, solved_score=args.stop_reward, e_rate=0.2, mute_rate=0.4, hidd_l=16, strategy=args.strategy, seed=args.seed)
        print('\n')
        start = time.time()
        agent.simulate()
        end = time.time()
        ...
        # Save the agent
        #
        agent.best_agent.save("trained_agents/knapsack_ga_rta_seed-{}_checkpoint-{}".format(str(args.seed), str(args.items)))
    #
