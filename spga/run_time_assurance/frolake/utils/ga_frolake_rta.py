...
class Generation:
    ...
    def __init__(self, env, continuous=False, n=20, std=.1, g=100, solved_score=190, e_rate=0.2, mute_rate=0.2, hidd_l=32, strategy=None, seed=None):
        ...
        self.strategy = strategy
        if strategy != None:
            print("Using {} to train fromg ga files...".format(strategy))
    ...
    def calc_fitness(self):
        ...
        for i in range(self.n):
            ...
            while True:
                ...
                # runtime assurance taken into account in the library
                # if self.strategy == "runtime":
                #     probe_state, unsafe = self.env.probe_step(action)
                #     if unsafe:
                #         action = self.env.get_safe_control()
                #         r = -1
                
                obs, reward, done, _ = self.env.step(action)
                agent.update_reward(reward + r)
                if done:
