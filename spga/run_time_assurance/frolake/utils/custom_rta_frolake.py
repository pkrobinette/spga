...
import numpy as np
from utils.custom_rta_biggerlake import BiggerLakeEnv
...
class FrozenLake(gym.Env):
    ...
    def probe_step(self, action):
        ...
        nrows = int(self.map_name.split("x")[0])
        operator = {
            0: -1,
            1: nrows,
            2: 1,
        ...
        # first column
        # if on the leftmost side, can't move left and will stay in current block
        if s%nrows == 0 and action == 0:
            # print("t", s, action)
            return s, 1
                
        # last colum
        # if on the rightmost side, can't move right and will stay in current block
        elif (s+1)%nrows == 0 and action == 2:
            # print("h", s, action)
            return s, 1
              
        #  top row
        elif s < nrows and action == 3:
            # print("e", s, action)
            return s, 1
        
        # bottom row
        elif s >= nrows**2 - nrows:
            # print("r", s, action)
            return s, 1
