...
def get_args():
    ...
    parser.add_argument("--stop_reward", type=float, default=0.95, help="Stopping reward criteria for training")
...
def get_ppo_trainer(args= None):
    ...
    if args:
        if vars(args).get("strategy", None) == "action_masking":
            print("Double CHECK: ", args.strategy)
        ...
        elif vars(args).get("strategy", None) == "runtime":
...
def main():
    ...
    for i in range(args.num_trials):
        ...
        #
        name = "frolake_ppo_rta_seed-{}_checkpoint-{}".format(str(args.seed), str(args.map.split("x")[0]))
    ...
    print()
    print()
    env = FrozenLake(env_config={"use_action_masking":False, "map_name":args.map})
