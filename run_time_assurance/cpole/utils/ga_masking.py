...
class Generation:
    ...
    def __init__(self, env, continuous=False, n=20, std=.1, g=100, solved_score=190, e_rate=0.2, mute_rate=0.2, hidd_l=32, strategy=None, seed=None):
        ...
        self.strategy = strategy
        if strategy == "runtime":
            print("Using X Threshold: ", env.x_threshold)
        ...
        for i in range(self.n):
            ...
            for k in range(500):
                ...
                if self.strategy == "runtime":
                    probe_step, unsafe = self.env.probe_step(action)
                    if unsafe: # smaller reward for needing to use rta module
                        self.ag_rewards[i] = agent.reward
                        break
                obs, reward, done, _ = self.env.step(action)
                if self.strategy == "action_masking" and 0 in obs["action_mask"]:
                    reward -= 10
